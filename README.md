...
On first run, `mdstack` creates a `mdstack.config.js` in your folder, and — if there isn't already an `index.md` — drops a starter `index.md` next to it so you get a working homepage immediately. Edit the config to customize the brand, theme, footer, and public site URL:
